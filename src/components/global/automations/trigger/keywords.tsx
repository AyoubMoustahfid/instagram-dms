...
const Keywords = ({ id }: Props) => {
    const {deleteMutation, keyword, onKeyPress, onVolumeChange} = useKeywords(id)
        ...
        <div className='bg-background-80 flex flex-col gap-y-3 p-3 rounded-xl'>
            ...
            <div className="flex flex-wrap justify-start gap-2 items-center">
                ...
                data?.data?.keywords.map(
                    ...
                    <div 
                        key={word.id}
                        className="bg-[#101010] flex items-center gap-x-2 capitalize text-text-secondary py-1 px-4 rounded-full"
                    >
                        <p>
                            {word.word}
                        </p>
