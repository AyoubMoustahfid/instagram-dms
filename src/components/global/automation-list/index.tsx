...
const AutomationList = (props: Props) => {
    ...
    const optimisticUiData = useMemo(() => {
        if (data?.status === 200 && latestVariable?.variables) {
        ...
        <div className="flex flex-col gap-y-3">
            {optimisticUiData?.data!.map((automation) => (
                <Link
                    ...
                    key={automation.id}
                    className="bg-[#1D1D1D] hove:opacity-80 transition duration-100 rounded-xl p-5 border-[1px] radial--gradient--automations flex border-[#545454]"
                >
